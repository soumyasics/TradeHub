...
const activateModeratorById = (req, res) => {
  Moderator.findByIdAndUpdate({ _id: req.params.id }, { isActive: true })
    .exec()
    .then(data => {
      res.json({
        status: 200,
...
const deactivateModeratorById = (req, res) => {
  Moderator.findByIdAndUpdate({ _id: req.params.id }, { isActive: false })
    .exec()
    .then(data => {
      res.json({
        status: 200,
...
const adminApproveModeratorById = (req, res) => {
  Moderator.findByIdAndUpdate({ _id: req.params.id }, { adminApproved: true,isActive:true })
    .exec()
    .then(data => {
      res.json({
        status: 200,
...
const forgotPassword = (req, res) => {
  Moderator.findOneAndUpdate({ email: req.body.email }, { password: req.body.password })
    .exec()
    .then(data => {
      if (data != null)
        res.json({
...
const resetPassword = async (req, res) => {
  ...
  if (pwdMatch) {
    await Moderator.findByIdAndUpdate({ _id: req.params.id }, { password: req.body.newpassword })
      .exec()
      .then(data => {
        if (data != null)
          res.json({
